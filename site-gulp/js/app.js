(() => {
    ...
    function slideToLoop(index = 0, speed = this.params.speed, runCallbacks = true, internal) {
        ...
        const swiper = this;
        let newIndex = index;
        if (swiper.params.loop) if (swiper.virtual && swiper.params.virtual.enabled) newIndex += swiper.virtual.slidesBefore; else newIndex = utils_elementIndex(swiper.slides.filter((slideEl => slideEl.getAttribute("data-swiper-slide-index") * 1 === newIndex))[0]);
    ...
    function loopFix({slideRealIndex, slideTo = true, direction, setTranslate, activeSlideIndex, byController} = {}) {
        ...
        if (typeof activeSlideIndex === "undefined") activeSlideIndex = utils_elementIndex(swiper.slides.filter((el => el.classList.contains("swiper-slide-active")))[0]); else activeIndex = activeSlideIndex;
    ...
    window.onload = function() {
        ...
        function documentActions(e) {
            ...
            if (window.innerWidth > 767.98 && isMobile.any()) {
                if (targetElement.classList.contains("menu__arrow")) targetElement.closest(".menu__list").classList.toggle("_hover");
                if (!targetElement.closest(".menu__list") && document.querySelectorAll(".menu__list._hover").length > 0) removeClasses(document.querySelectorAll(".menu__list._hover"), "_hover");
            }
        }
    };
    window["FLS"] = true;
